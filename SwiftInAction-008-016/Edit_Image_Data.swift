...
class Edit_Image_Data {
    ...
    var tiezhi_collectionViewSize:CGSize{
        let width = (Int(screenBounds.width)-40-5*5)/4///每个Image贴图占的大小
        let widthSum:CGFloat!
        widthSum = CGFloat(Edit_Image_tiezhi.count*width)+5+30
    ...
    var biaoqian_collectionViewSize:CGSize{
        let width = (Int(screenBounds.width)-40-5*5)/4///每个Image标签占的大小
        let widthSum:CGFloat!
        widthSum = CGFloat(Edit_Image_biaoqian.count*width)+5+30
        return CGSize(width: widthSum,height: CGFloat(0))
    }
    
    var Edit_Image_lvjing=["yindao2.png","yindao3.jpg","yindao4.png","yindao3.jpg","yindao4.png","卡背.jpg"]
    var lvjing_collectionViewSize:CGSize{
        let width = (Int(screenBounds.width)-40-5*5)/4///每个Image标签占的大小
        let widthSum:CGFloat!
        widthSum = CGFloat(Edit_Image_lvjing.count*width)+5+30
        return CGSize(width: widthSum,height: CGFloat(0))
    }
